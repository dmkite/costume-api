...
function getOneTag(req, res, next) {
    ...
    const correctTag = model.getOneTag(correctCostume.id, tId)
...
function createTag(req, res, next) {
    ...
    const createdTag = model.createTag(newTag, correctCostume.id)
...
function editTag(req, res, next) {
    ...
    const editedTag = model.editTag(correctCostume.id, tId, edits)
    if (!editedTag) return next({ status: 404, message: `No tag at id '${id}'` })
...
function deleteTag(req, res, next) {
    const { correctCostume } = req.body
    const { tId } = req.params
    let deletedTag = model.deleteTag(correctCostume.id, tId)
